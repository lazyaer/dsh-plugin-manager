...
window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    function renderMarket() {
      ...
      if (state.market.length === 0) {
        if (state.marketLoading) {
          marketListEl.appendChild(el('li', { class: 'dpm-empty', text: '正在从 npm registry 加载插件…' }))
        } else if (state.error) {
          var errorItem = el('li', { class: 'dpm-empty' })
          errorItem.appendChild(el('div', { text: '加载失败：' + state.error }))
          var retry = el('button', { type: 'button', class: 'dpm-refresh', text: '重试' })
          retry.addEventListener('click', refreshMarket)
          errorItem.appendChild(retry)
          marketListEl.appendChild(errorItem)
        } else {
          marketListEl.appendChild(el('li', { class: 'dpm-empty', text: '没有找到匹配的 DSH / harness 插件。试试搜索关键词，或检查网络后重试。' }))
        }
        return
      }
      for (var i = 0; i < state.market.length; i++) {
        ...
        if (item.downloads) meta.push('⬇ ' + item.downloads + '/月')
    ...
    function mount() {
      ...
      head.appendChild(el('span', { class: 'dpm-panel-title', text: '插件市场' }))
      head.appendChild(el('span', { class: 'dpm-panel-sub', text: '浏览安装 npm DSH 插件 · 管理已安装插件 · 下载审计（来源与时间）' }))
      ...
      var tabPlugins = el('button', { type: 'button', class: 'dpm-tab dpm-tab-on', text: '已安装插件' })
      ...
      var marketSearch = el('input', { type: 'search', class: 'dpm-search', placeholder: '搜索 npm 上的 DSH / harness 插件', value: state.marketQuery })
      ...
      tabMarket.addEventListener('click', function () {
        ...
        if (!state.marketLoading) refreshMarket()
      })
