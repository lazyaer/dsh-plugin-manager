...
/** Accepts `github:owner/repo` (optionally `#ref`), never shell metacharacters. */
export function isMarketSpec(spec) {
  return typeof spec === 'string' && MARKET_SPEC_RE.test(spec)
}
...
/**
 * Fetch the public `dsh-plugin` GitHub topic (optionally filtered by `query`).
 * Uses the GitHub search API; no token required for modest use.
 */
export async function fetchMarketplace({ query = '', page = 1, perPage = 20 } = {}) {
  ...
  url.searchParams.set('per_page', String(Math.min(100, Math.max(1, Number(perPage) || 20))))
  ...
  if (!response.ok) {
    throw new Error(`GitHub search failed: HTTP ${response.status}${response.status === 403 ? ' (rate limit or network blocked)' : ''}`)
  }
  ...
  return {
    ...
    items: items.map((item) => ({
      ...
      installSpec: item.full_name ? `github:${item.full_name}` : null,
    })),
  }
}
...
export async function listMarketplace(profileDir, options = {}) {
  ...
  for (const item of data.items) {
    const normalized = normalizeGitHubRepo(item.url ?? item.fullName)
...
export async function installMarketPlugin(profileDir, anchors, profileName, spec, options = {}) {
  if (!isMarketSpec(spec)) {
    return { ok: false, error: 'invalid marketplace spec; expected github:owner/repo' }
  }
