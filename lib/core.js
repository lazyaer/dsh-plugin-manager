...
const MARKET_SPEC_RE = /^github:[A-Za-z0-9](?:[A-Za-z0-9._-]*[A-Za-z0-9])?\/[A-Za-z0-9](?:[A-Za-z0-9._-]*[A-Za-z0-9])?(?:#[A-Za-z0-9._/-]+)?$/
const PROFILE_NAME_RE = /^[A-Za-z0-9._-]+$/

/**
 * Accepts a marketplace install spec: `github:owner/repo` (optionally `#ref`)
 * or a plain npm package name (e.g. `dsh-foo`, `@scope/dsh-foo`).
 * Never accepts shell metacharacters.
 */
export function isMarketSpec(spec) {
  return typeof spec === 'string' && (MARKET_SPEC_RE.test(spec) || PACKAGE_NAME_RE.test(spec))
}

/** Normalize a GitHub URL/repository string to `owner/repo` (lowercase) when possible. */
...
/**
 * Fetch DSH/harness plugins published on the npm registry (keyword
 * `dsh-plugin`, optionally filtered by `query`). These are real npm packages,
 * so `dsh plugin --profile <profile> add <name>` can install them directly.
 */
export async function fetchMarketplace({ query = '', page = 1, perPage = 20 } = {}) {
  const size = Math.min(100, Math.max(1, Number(perPage) || 20))
  const pageNumber = Math.max(1, Number(page) || 1)
  const from = (pageNumber - 1) * size
  const text = ['keywords:dsh-plugin']
  if (typeof query === 'string' && query.trim() !== '') text.push(query.trim())
  const url = new URL(NPM_SEARCH_API)
  url.searchParams.set('text', text.join(' '))
  ...
  try {
    ...
    throw new Error(`npm registry search failed: ${error?.name === 'TimeoutError' ? 'timeout' : error?.message ?? error}`)
  }
  if (!response.ok) {
  ...
  return {
    ...
    items: objects.map((obj) => {
      ...
      const url = repository && /^https?:\/\//i.test(repository) ? repository : (links.npm ?? null)
      return {
        fullName: pkg.name ?? null,
        name: pkg.name ?? null,
        owner: null,
        url,
        ...
        stars: 0,
        forks: 0,
        updatedAt: pkg.date ?? obj.updated ?? null,
        defaultBranch: null,
        version: pkg.version ?? null,
        downloads: Number(obj.downloads?.monthly) || 0,
        installSpec: pkg.name ?? null,
      }
    }),
  }
}

/** Set of normalized GitHub `owner/repo` values for installed plugin packages. */
export function installedMarketplaceSet(profileDir) {
  const set = new Set()
  for (const info of installedPlugins(profileDir).values()) {
    for (const value of [info.repository, info.homepage]) {
...
/** Fetch marketplace entries and annotate which are already installed. */
export async function listMarketplace(profileDir, options = {}) {
  const data = await fetchMarketplace(options)
  const installed = installedMarketplaceSet(profileDir)
  const installedNames = new Set(installedPlugins(profileDir).keys())
  for (const item of data.items) {
    const normalized = normalizeGitHubRepo(item.url ?? '')
    item.installed = normalized ? installed.has(normalized) : false
    if (!item.installed && item.name) item.installed = installedNames.has(item.name)
  }
  return data
}

function runDshAdd(profileDir, profileName, spec, timeoutMs = 300_000) {
  return new Promise((resolve) => {
    const isWin = process.platform === 'win32'
    const args = ['plugin', '--profile', profileName, 'add', spec]
    const env = { ...process.env, CI: 'true' }
    const child = isWin
      ? spawn('cmd.exe', ['/d', '/s', '/c', `dsh ${args.map((arg) => `"${String(arg).replace(/"/g, '""')}"`).join(' ')}`], { cwd: profileDir, windowsHide: true, env })
      : spawn('dsh', args, { cwd: profileDir, windowsHide: true, env })
    let out = ''
    let err = ''
    const outBox = { value: out }
    const errBox = { value: err }
    child.stdout?.on('data', (chunk) => {
      outBox.value += chunk
      if (outBox.value.length > 12000) outBox.value = outBox.value.slice(-12000)
    })
    child.stderr?.on('data', (chunk) => {
      errBox.value += chunk
      if (errBox.value.length > 12000) errBox.value = errBox.value.slice(-12000)
    })
    const timer = setTimeout(() => {
      child.kill()
      resolve({ ok: false, error: 'timeout', code: null, out: outBox.value, err: errBox.value })
    ...
    child.on('error', (error) => {
      ...
      resolve({
        ok: false,
        error: error?.code === 'ENOENT' ? 'dsh-not-found' : String(error?.message ?? error),
        code: null,
        out: outBox.value,
        err: errBox.value,
      })
...
/**
 * Install a marketplace plugin by running the canonical dsh CLI command:
 * `dsh plugin --profile <profile> add <spec>` in the profile directory.
 * The spec is strictly validated before it reaches the command line.
 */
export async function installMarketPlugin(profileDir, anchors, profileName, spec, options = {}) {
  if (!isMarketSpec(spec)) {
    return { ok: false, error: 'invalid marketplace spec; expected github:owner/repo or an npm package name' }
  }
  if (!PROFILE_NAME_RE.test(profileName)) {
    return { ok: false, error: 'refusing to run dsh with an unsafe profile name' }
  }
  const result = await runDshAdd(profileDir, profileName, spec, options.timeoutMs)
  const output = (result.err || result.out || '').trim().slice(-4000)
  if (!result.ok) {
    return {
      ok: false,
      spec,
      command: `dsh plugin --profile ${profileName} add ${spec}`,
      error: result.error === 'timeout' ? 'dsh install timed out' : result.error === 'dsh-not-found' ? 'dsh command not found; make sure the DSH CLI is on PATH' : result.error ?? `dsh exit ${result.code}`,
      output,
    }
  }
  let auditEvents = []
  try {
    ...
    // audit is best-effort; installation already succeeded
  }
  return {
    ok: true,
    spec,
    ...
    output,
    auditEvents,
    note: 'installed through dsh CLI; restart dsh web to load the new plugin',
  }
}
